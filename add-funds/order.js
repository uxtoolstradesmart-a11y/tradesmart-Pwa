...
export function renderOrder(app,instrument){
 ...
 function refresh(){
  ...
  app.querySelector('#order-helper').textContent=fno?(s.mode==='Lots'?`Qty: ${q}`:`Lots: ${q/LOT_SIZE}`):s.mode==='Amount'?`Qty: ${Number.isFinite(q)?q:0}`:`Approx : ₹${number(Number.isFinite(q*r)?q*r:0)}`;
 ...
 <div class="order-field"><div class="order-field-title"><span>Price <button class="order-price-help" aria-label="About price" type="button">${icon('price')}</button></span>${pill('Price type',['Limit','Market'],s.pricing)}</div>${stepper('price','Limit price',Number(s.price).toFixed(2),s.pricing==='Market')}<p class="order-tick" ${s.pricing==='Market'?'style="visibility:hidden"':''}>Tick: 0.05</p></div></section>
